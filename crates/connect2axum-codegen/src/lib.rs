...
pub fn try_generate(request: &CodeGeneratorRequest) -> CodegenResult<CodeGeneratorResponse> {
    ...
    let files = request
        .file_to_generate
        .iter()
        .map(|file_name| generate_placeholder_file(&ir, file_name))
...
fn generate_placeholder_file(
    ...
    let content = placeholder_content(proto_file);
...
fn placeholder_content(proto_file: &ProtoFile) -> String {
    let mut content = format!(
        "// @generated by connect2axum.\n\
         // Phase 3 descriptor IR placeholder for {}.\n\
         // package: {}\n",
